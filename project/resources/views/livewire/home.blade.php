<div>
    ...
    {{--main part--}}
    <main class="w-full bg-[url('{{ asset( 'storage/' . $mainContent->image ) }}')] bg-cover relative flex justify-center items-center">
        <div class="absolute h-full w-full top-0 left-0 bg-[{{ $mainContent->color }}] z-10"></div>
        <div class="container flex flex-col justify-center items-center h-screen w-full">
            <h1 class="text-5xl font-semibold text-white leading-13.5 capitalize w-2xl text-center z-20 mt-20">{!! $mainContent->heading !!}</h1>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.company-profile-showcase').slick({
                dots: false,
                infinite: true,
                arrows: false,
                slidesToShow: 10,
